...
def set_seed(seed: int) -> None:
    """
    Seeds Python, NumPy, and PyTorch (incl. CUDA) RNGs for reproducible runs.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def load_splits(data_dir: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Loads train/val/test CSVs from data_dir, validating required columns exist.
    """
...
def main(config_path: str, data_dir: str) -> None:
    """
    Runs the full pipeline: load config/data, build datasets & model, train, print results.
    """
    load_dotenv(".env")
