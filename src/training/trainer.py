...
class SentimentTrainer:
    ...
    def train(
        ...
        """Fine-tune the model, evaluate on val/test, and log everything to MLflow."""
    ...
    def _build_training_args(self) -> TrainingArguments:
        """Translate the config dict into a HuggingFace TrainingArguments object."""
    ...
    def _build_mlflow_params(
        ...
        """Assemble the hyperparameters and dataset sizes to log as MLflow params."""
        t = self.config["training"]
    ...
    def _log_step_metrics(hf_trainer: Trainer) -> None:
        """Push per-epoch eval/loss entries from the Trainer's log history to MLflow."""
        loggable = {
            "eval_accuracy",
            "eval_f1_weighted",
            "eval_f1_macro",
    ...
    def _validate_config(self) -> None:
        """Raise KeyError if any required config section or key is missing."""
